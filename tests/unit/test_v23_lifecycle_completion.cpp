...
TEST_CASE("ArtifactKind — labels", "[v23][p03][label]")
{
    ...
    CHECK(std::string(artifact_kind_label(ArtifactKind::kNotebook)) == "Notebook");
    CHECK(std::string(artifact_kind_label(ArtifactKind::kCanvas)) == "Canvas");
    CHECK(std::string(artifact_kind_label(ArtifactKind::kWorkspace)) == "Workspace");
    CHECK(std::string(artifact_kind_label(ArtifactKind::kSession)) == "Session");
}
...
TEST_CASE("LifecycleCompletionTracker — query by kind", "[v23][p03][query]")
{
    ...
    LifecycleCoverageItem notebook;
    notebook.artifact_kind = ArtifactKind::kNotebook;
    tracker.add_item(std::move(notebook));

    CHECK(tracker.items_by_kind(ArtifactKind::kTextFile).size() == 1);
    CHECK(tracker.items_by_kind(ArtifactKind::kNotebook).size() == 1);
    CHECK(tracker.items_by_kind(ArtifactKind::kCanvas).empty());
}
...
TEST_CASE("LifecycleCompletionTracker — gap report", "[v23][p03][report]")
{
    ...
    LifecycleCoverageItem nb_gap;
    nb_gap.artifact_kind = ArtifactKind::kNotebook;
    tracker.add_item(std::move(nb_gap));

    LifecycleCoverageItem canvas_gap;
    canvas_gap.artifact_kind = ArtifactKind::kCanvas;
    tracker.add_item(std::move(canvas_gap));

    auto report = tracker.gap_report();
    CHECK(report.total_items == 3);
    CHECK(report.complete == 1);
    CHECK(report.incomplete == 2);
    CHECK(report.notebook_gaps == 1);
    CHECK(report.canvas_gaps == 1);
...
TEST_CASE("LifecycleCompletionTracker — Markdown export", "[v23][p03][export]")
{
    ...
    LifecycleCoverageItem gap;
    gap.artifact_kind = ArtifactKind::kNotebook;
...
TEST_CASE("Integration — full lifecycle tracker lifecycle", "[v23][p03][integration]")
{
    ...
    // Incomplete notebook lifecycle
    LifecycleCoverageItem nb_create;
    nb_create.artifact_kind = ArtifactKind::kNotebook;
    nb_create.stage = LifecycleStage::kCreation;
    nb_create.is_authoritative = true;
    nb_create.is_shell_owned = true;
    nb_create.has_stable_id = true;
    tracker.add_item(std::move(nb_create));

    LifecycleCoverageItem nb_save;
    nb_save.artifact_kind = ArtifactKind::kNotebook;
    nb_save.stage = LifecycleStage::kSave;
    // Missing shell ownership
    nb_save.is_authoritative = true;
    tracker.add_item(std::move(nb_save));
