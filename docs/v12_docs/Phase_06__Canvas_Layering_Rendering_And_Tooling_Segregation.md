# Phase 06: Canvas Layering Rendering And Tooling Segregation

## Goal
Separate canvas domain model, tool orchestration, and rendering pathways.

## Why This Phase Now
Canvas has high feature density and needs clearer split between data model, interaction tools, and frame rendering for scalability.
...
## Tasks (22)
1. Run a focused architecture inventory for canvas model/tool/render separation and capture current ownership gaps.
2. Identify the top coupling points and rank them by change-risk and blast radius.
3. Define explicit module responsibilities and non-responsibilities for canvas model/tool/render separation.
...
7. Split oversized classes/functions in canvas model/tool/render separation into cohesive components.
...
10. Standardize naming and folder placement for canvas model/tool/render separation artifacts.
